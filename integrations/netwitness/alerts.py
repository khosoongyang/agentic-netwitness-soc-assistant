"""NetWitness alert-response parsing & distillation (pure helpers).

Canonical location for NetWitness Respond "/rest/api/incidents/{id}/alerts"
response parsing and alertMeta distillation - moved here from the former
root nw_alerts.py during Phase 9 consolidation (content unchanged, only the
location and the public wrapper names below are canonical now).

These functions are PURE (stdlib only - no UI framework, session state,
globals or network), so they unit-test offline.
...
alert_items() / has_more_alerts() / enrich_incident() are the public,
non-underscore names used by client.py; workflow/engine.py imports
_merge_alert_digest directly (both are the same functions this file defines).
"""
...
def _extract_alert_items(payload) -> list:
    ...
    [FYP-USED-BY] the per-incident alerts pagination loop, called once
...
def _alerts_has_more(payload, page: int) -> bool:
    ...
    [FYP-USED-BY] the per-incident alerts pagination loop, evaluated
...
def _alerts_error_hint(code: int) -> str:
    ...
    [FYP-USED-BY] the alerts-fetch failure path (feeds the diagnostic
...
def _merge_alert_digest(inc: dict) -> None:
    ...
    [FYP-USED-BY] workflow/engine.py (imported directly, called after each
    incident's alerts finish paginating, and after an APIRetrieval FETCH-API
    enrichment) and integrations.netwitness.client.py (via enrich_incident())."""
...
def _alerts_fetch_warning(inc: dict) -> str:
    ...
    upstream by the fetch loop using _alerts_error_hint()'s hint text)
