...
# Temporary migration reference: run with
# `streamlit run scripts/legacy_streamlit_app.py` until Flask feature parity.
# The original file lived at the repository root, so retain that import and
# data-path base after moving it under scripts/.
import sys as _legacy_sys
from pathlib import Path as _LegacyPath

LEGACY_PROJECT_ROOT = _LegacyPath(__file__).resolve().parents[1]
if str(LEGACY_PROJECT_ROOT) not in _legacy_sys.path:
    _legacy_sys.path.insert(0, str(LEGACY_PROJECT_ROOT))
...
def _maybe_reload_agent_modules():
    ...
    watched = {
        "soc_triage_agent.soc_triage_agent":
            LEGACY_PROJECT_ROOT / "soc_triage_agent" / "soc_triage_agent.py",
        "soc_workflow": LEGACY_PROJECT_ROOT / "soc_workflow.py",
    }
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
ENV_FILE = LEGACY_PROJECT_ROOT / ".env"
...
SOC_DB_DIR = LEGACY_PROJECT_ROOT / "soc_db"
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if _is_dev:
        ...
        if cert_upload is not None:
            certs_dir = LEGACY_PROJECT_ROOT / "certs"
